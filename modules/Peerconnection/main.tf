...
resource "aws_vpc_peering_connection" "newpeerconnection" {
  peer_owner_id =  "713942048904"
  peer_vpc_id   =  aws_default_vpc.default.id
  vpc_id        =  var.accepter_qa_vpcid

 # cidr_block = aws_default_vpc.default.cidr_block 
  auto_accept   = true

  tags = {
    Name = var.peerconnectionName.peername
  }
}
...
resource "aws_route" "newvpcroute" {
  ...
  vpc_peering_connection_id =  aws_vpc_peering_connection.newpeerconnection.id
}
